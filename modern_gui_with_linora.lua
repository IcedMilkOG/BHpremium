-- Modern GUI System with Code-Based Save System using Linora Library
...
print("=== MODERN FUNCTIONAL GUI WITH CODE SAVES STARTING (LINORA VERSION) ===")

-- Load Linora Library
local Linora = loadstring(game:HttpGet("https://raw.githubusercontent.com/Jeserik/Linora/main/Linora"))()
...
-- Initialize Linora Window
local Window = Linora:CreateWindow({
    Title = "BIGHACK - PREMIUM [v1.0.0]",
    Description = "Modern GUI System with Code-Based Save System",
    Theme = "Dark",
...
CombatSection:CreateToggle({
    Name = "Player ESP",
    CurrentValue = features.playerESP,
    Flag = "PlayerESP",
...
CombatSection:CreateToggle({
    Name = "Head Circles",
    CurrentValue = features.headCircles,
    Flag = "HeadCircles",
    Callback = function(Value)
...
CombatSection:CreateToggle({
    Name = "Armor ESP",
    CurrentValue = features.armorESP,
    Flag = "ArmorESP",
    Callback = function(Value)
...
CombatSection:CreateToggle({
    Name = "Snaplines",
    CurrentValue = features.snaplines,
    Flag = "Snaplines",
    Callback = function(Value)
...
CombatSection:CreateToggle({
    Name = "Distance",
    CurrentValue = features.distance,
    Flag = "Distance",
    Callback = function(Value)
...
CombatSection:CreateToggle({
    ...
    CurrentValue = features.selectionBoxes,
    Flag = "SelectionBoxes",
    Callback = function(Value)
...
local CrosshairSection = VisualsTab:CreateSection("Crosshair Settings")

CrosshairSection:CreateToggle({
    Name = "Crosshair",
    CurrentValue = features.crosshair,
    Flag = "Crosshair",
    Callback = function(Value)
...
CrosshairSection:CreateDropdown({
    ...
    Options = {"Plus", "X-Shape", "Circle", "Dot", "Arrow", "Target", "Star", "Diamond"},
    CurrentOption = visualSettings.crosshairStyle,
    Flag = "CrosshairStyle",
    Callback = function(Option)
        visualSettings.crosshairStyle = Option
        updateCrosshairStyle()
    end,
})
...
CrosshairSection:CreateSlider({
    ...
    Range = {8, 40},
    Increment = 1,
    CurrentValue = visualSettings.crosshairSize,
    Flag = "CrosshairSize",
    Callback = function(Value)
...
CrosshairSection:CreateColorPicker({
    ...
    Color = Color3.fromRGB(visualSettings.crosshairColor.r, visualSettings.crosshairColor.g, visualSettings.crosshairColor.b),
    Flag = "CrosshairColor",
    Callback = function(Value)
        visualSettings.crosshairColor = {r = math.floor(Value.R * 255), g = math.floor(Value.G * 255), b = math.floor(Value.B * 255)}
        updateCrosshairColors()
    end
})
...
CrosshairSection:CreateToggle({
    Name = "Crosshair Spin",
    CurrentValue = features.crosshairSpin,
    Flag = "CrosshairSpin",
    Callback = function(Value)
...
CrosshairSection:CreateSlider({
    ...
    Range = {10, 200},
    Increment = 5,
    CurrentValue = visualSettings.crosshairSpinSpeed,
...
FOVSection:CreateSlider({
    ...
    Range = {50, 300},
    Increment = 5,
    CurrentValue = visualSettings.fovCircleSize,
    Flag = "FOVSize",
    Callback = function(Value)
...
FOVSection:CreateColorPicker({
    ...
    Color = Color3.fromRGB(visualSettings.fovCircleColor.r, visualSettings.fovCircleColor.g, visualSettings.fovCircleColor.b),
    Flag = "FOVCircleColor",
    Callback = function(Value)
        visualSettings.fovCircleColor = {r = math.floor(Value.R * 255), g = math.floor(Value.G * 255), b = math.floor(Value.B * 255)}
    end
})

local ParticleSection = VisualsTab:CreateSection("Particle Effects")

ParticleSection:CreateToggle({
    Name = "Particle Trails",
    CurrentValue = features.particles,
    Flag = "Particles",
    Callback = function(Value)
...
MiscSection:CreateToggle({
    Name = "Status Display",
    CurrentValue = features.statusLabel,
    Flag = "StatusLabel",
    Callback = function(Value)
...
local CodeGenSection = SavesTab:CreateSection("Generate Configuration Code")

CodeGenSection:CreateButton({
    Name = "Generate Code",
    Callback = function()
        ...
        if setclipboard then
            ...
            Linora:Notify({
                ...
                Content = "Configuration code copied to clipboard!",
                Duration = 3
            })
        else
            Linora:Notify({
...
local QuickCodesSection = SavesTab:CreateSection("Quick Configuration Codes")

QuickCodesSection:CreateButton({
    Name = "COMBAT - Combat focused setup",
    Callback = function()
        local success, message = applyConfigFromCode("COMBAT")
        Linora:Notify({
...
QuickCodesSection:CreateButton({
    Name = "VISUAL - Visual effects setup",
    Callback = function()
        local success, message = applyConfigFromCode("VISUAL")
        Linora:Notify({
            Title = success and "Success" or "Error",
            Content = message,
            Duration = 3
...
QuickCodesSection:CreateButton({
    Name = "STEALTH - Minimal visibility setup",
    Callback = function()
        local success, message = applyConfigFromCode("STEALTH")
        Linora:Notify({
            Title = success and "Success" or "Error",
            Content = message,
            Duration = 3
...
QuickCodesSection:CreateButton({
    Name = "RAINBOW - Full effects setup",
    Callback = function()
        local success, message = applyConfigFromCode("RAINBOW")
        Linora:Notify({
            Title = success and "Success" or "Error",
            Content = message,
            Duration = 3
...
QuickCodesSection:CreateButton({
    Name = "MINIMAL - Performance focused",
    Callback = function()
        local success, message = applyConfigFromCode("MINIMAL")
        Linora:Notify({
            Title = success and "Success" or "Error",
            Content = message,
            Duration = 3
...
local LoadCodeSection = SavesTab:CreateSection("Load Configuration Code")

LoadCodeSection:CreateInput({
    Name = "Configuration Code",
    PlaceholderText = "Paste your code here...",
    RemoveTextAfterFocusLost = false,
    Callback = function(Text)
        if Text and Text ~= "" then
            local success, message = applyConfigFromCode(Text)
            Linora:Notify({
                Title = success and "Success" or "Error",
                Content = message,
                Duration = 3
...
print("GUI created with Linora, now creating all visual elements...")

-- CREATE ALL ORIGINAL VISUAL ELEMENTS (same as original script)
...
statusLabel.Text = "STARTING UPDATE LOOP"

local connection = RunService.Heartbeat:Connect(function()
    pcall(update)
end)

print("=== CODE-BASED SAVE SYSTEM LOADED SUCCESSFULLY WITH LINORA GUI ===")
